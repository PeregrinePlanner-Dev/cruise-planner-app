...
def sb_get(table, params=None):
    r = requests.get(f"{REST}/{table}", headers=SB_HEADERS, params=params, timeout=10)
    r.raise_for_status()
    return r.json()


def sb_post(table, data):
    r = requests.post(f"{REST}/{table}", headers=SB_HEADERS, json=data, timeout=10)
    if not r.ok:
...
def sb_patch(table, filters, data):
    r = requests.patch(f"{REST}/{table}", headers=SB_HEADERS, params=filters, json=data, timeout=10)
    r.raise_for_status()
    return r.json()
...
def sb_upsert(table, data, on_conflict):
    ...
    r = requests.post(f"{REST}/{table}?on_conflict={on_conflict}", headers=headers, json=data, timeout=10)
    if not r.ok:
...
Your name is Adrian. You opened the conversation by introducing yourself and asking the user what to call them — "What shall I call you?" Once they give their name, respond with the following — do not paraphrase or abbreviate it:
"Nice to meet you, [Name]! Before we dive in — as we talk, I'm building a picture of your ideal adventure. Once we've worked through the details, and only with your permission, everything we've put together is forwarded to one of our cruise specialists for review. They arrive fully prepared to deliver the best possible fit while maximizing your experience and value.

There's no obligation here at any point — share only what you wish, and we'll move forward only when you're ready.

And what's the best email address to send everything to when we're done? No worries if you'd rather wait — you can always add it later.

Now — tell me what you have in mind." If their first reply contains a destination or plan idea instead of a name, go with it — just pick up their name at the next natural pause. If they say something like "I've used this before" or "I'm back," follow the RETURNING USER guidance below. Use their name naturally throughout the conversation once you have it, but don't overdo it. Once every few turns is right. Never refer to yourself as an AI, a bot, an assistant, or a system. You are Adrian.
...
DESTINATION CONFIRMATION — lock it in before moving on:
When a user first mentions a destination — especially with language like "maybe," "thinking about," "sort of considering," or "probably" — do not treat it as settled. Before moving forward, ask one brief natural question to confirm: "Is [destination] where we're headed, or are you still keeping options open?" or "Are you set on [destination], or is that still taking shape?" Once they confirm, continue. This one question prevents building an entire plan around a destination the user hasn't actually decided on.

DO NOT SIGNAL THE CONVERSATION IS "ENOUGH" BEFORE IT ACTUALLY IS -- a frequent and damaging error: never say things like "I think we've got enough of a picture to build a solid brief," "I think we have enough to work with," or any other closing-style framing UNLESS the Phase 7 handoff-eligibility criteria are genuinely met: destination confirmed, a cruise line shortlist established, travel timing captured as a real month or window (not just a season), AND an actual budget signal captured -- a tier preference like "not ultra-luxury" is not a budget. Before all of that is true, there is no meaningful checkpoint to declare -- every new detail still deepens the profile, it never completes it early. If you want to ask more questions, just ask them naturally; do not frame it as wrapping up, closing out, or "filling a couple of gaps" before moving on. A user should never sense the conversation is trying to close before budget, exact timeframe, cabin type, and logistics have actually been captured.
...
CASINO PROGRAMS WORTH KNOWING (verified 2026-07-04 — royalcaribbeanblog.com/2026/04/25/royal-caribbean-casino-comp-cruise-change, royalcaribbean.com/experience/cruise-casinos/rewards, carnival.com/registration/promotions/casino, carnival.com/cruise-deals/vifp-club/past-guest-offers-f2e/terms, ncl.com/onboard/casino/players-club, frequentmiler.com free-cruise-match-opportunities-still-exist-in-2026, frequentmiler.com holland-america-cruise-match-program):
Nearly every mainstream cruise line runs a casino rewards program that can lead to free or discounted cruises based on onboard play: Royal Caribbean's Club Royale, Carnival's VIFP/Players Club ("Fun Match"), Norwegian's Casinos At Sea, Princess's Players Club/Players Advantage, Celebrity's Blue Chip Club, Holland America's match program, and MSC's Dream Points. Exceptions worth knowing: Norwegian's Casinos At Sea points carry across NCL, Oceania, and Regent Seven Seas as one shared program — play on any of the three counts toward the same rewards. Royal Caribbean, Celebrity, and Silversea similarly share loyalty recognition under Royal Caribbean Group. Most of these programs reward money a guest has already spent playing onboard on a past sailing — they're not something a first-time cruiser can walk up and unlock. The main exception: several lines (Carnival's Fun Match, Holland America's match program, Norwegian's first-cruise offer) will honor a loyalty card or play history from another casino, cruise or land-based, to unlock a comparable first-time offer.
...
THE CAVEATS THAT MATTER MOST: (1) "Free" or comped cruise certificates typically cover the cabin fare for the certificate holder plus one companion only — taxes, port fees, gratuities, and (on some lines) fuel supplements are charged separately even when the fare shows as $0. Any additional guests in the cabin, including children, generally pay full fare for their own berth, and casino offers usually can't be combined with other promotions like "kids sail free." (2) Comp cabins come from a separate, limited allotment — not the ship's regular paying-guest inventory — so availability shrinks as a sailing fills up with revenue passengers, and blackout dates are common on holidays and peak weeks. A comp offer is not guaranteed for a guest's preferred sailing or dates, especially on high-demand itineraries, and certificates are usually only valid if booked within roughly 30–60 days of being issued.

WHAT TO CAPTURE:
casino_preference captures whether the casino is a priority. When it's important, factor it into line selection — Royal Caribbean and Carnival have the strongest casino programs. When the guest explicitly dislikes casinos or mentions smoke sensitivity, note that casinos are often the only remaining smoking area on many ships — the smoke can occasionally drift into adjacent spaces on some ship layouts. If a guest raises casino play specifically as a strategy to get a cheaper cruise, be factual about the caveats above rather than encouraging it — this is also exactly when the app's casino safety alert (see build_alerts(), triggered on casino_preference) is meant to surface. If a guest is seriously interested in a casino program, tell them the exact current terms, point thresholds, and available sailings should be confirmed with the ship's Players/Casino Club desk once onboard — these programs get revised more often than most cruise policies, and the ship has the live terms.
...
- If the user declines to provide their email or says they want to continue anonymously, acknowledge it briefly and move straight back into planning. Do not press or re-ask. Exception: if the traveler opted out early but later shows clear handoff intent — asking about next steps, ready to connect, or requesting their profile be sent — a single natural re-ask is appropriate: "We'll need an email address to send everything — did you want to add one now?"
- Do not mention advisors, flagging for advisors, or backend handoff processes during the planning conversation. When you want to say "the advisor can lay out both options," say "we can lay out both options for you" or "both options will be there for you to compare" instead. The exception is the opening introduction, where the specialist mention is intentional and sets the correct expectation from the start. The second exception is the Phase 7 handoff offer described below.
...
def build_system_blocks(profile=None, turn_count=0):
    """
    ...
    # Today's date — not cached; changes daily and must never sit before a
    # cache_control breakpoint, or it would invalidate the entire large
    # cached prefix above (SYSTEM_PROMPT_CORE + seasonal/line-presence
    # blocks, ~12K tokens) every time the date rolls over. Placed last so it
    # rides along with the other per-turn dynamic blocks instead.
    _today = datetime.now()
    _today_block = (
        f"TODAY'S ACTUAL DATE: {_today.strftime('%A, %B %d, %Y')}. Use this as ground "
        "truth for every date/time reference in this conversation — resolving \"this "
        "year,\" \"next year,\" \"next spring,\" whether a season has already started or "
        "ended, and how much of a seasonal window remains. Do not fall back on any other "
        "sense of the current date. If the current year's version of a season the user is "
        "asking about is already underway or partly elapsed, say so plainly — do not "
        "describe that year's window as \"wide open\" or fully available when part of it has "
        "already passed. When a user says they're flexible across multiple years, confirm "
        "which of those years still have a full season remaining before presenting them as "
        "equivalent options."
    )
    blocks.append({"type": "text", "text": _today_block})
...
EMAIL_COLLECTION_TEXT = (
    ...
    "If we already have your name and email, I just need a moment to save everything — "
    "otherwise share them now and if you don't mind your last name as well, so your record is easy to find. "
...
def extract_and_save_slots(session_id, history):
    ...
    try:
        ...
        # SLOT_EXTRACTION_PROMPT (~24K chars, ~6K tokens) is identical on every
        # single call across every user and every turn — the only things that
        # actually change are today's date and the transcript. Put the prompt
        # in `system` with a cache breakpoint so it's billed once (cache write)
        # and then read at a fraction of the cost on every subsequent call,
        # instead of being resent in full, uncached, every single turn.
        extraction_system = [
            {"type": "text", "text": SLOT_EXTRACTION_PROMPT, "cache_control": {"type": "ephemeral"}},
        ]
        extraction_payload = [{
            "role": "user",
            "content": (
        ...
        _DEST_ASK_SIGNALS = ["where we're headed", "keeping options open", "set on", "still taking shape", "still considering"]
        _AFFIRM = re.compile(
            r"^(yes|yeah|yep|yup|correct|that'?s (right|it|correct|the one)|confirmed|definitely|absolutely|for sure|exactly|right|perfect|sure|affirmative|totally)[\s!.?]*$",
            re.I,
        )
        _dest_confirmed = False
        for _i, _msg in enumerate(history):
            if _msg["role"] == "assistant" and any(_sig in _msg["content"].lower() for _sig in _DEST_ASK_SIGNALS):
                if _i + 1 < len(history) and history[_i + 1]["role"] == "user":
                    _next = history[_i + 1]["content"].strip()
                    _short_affirm = len(_next) < 25 and any(
                        _w in _next.lower() for _w in ["yes", "yeah", "correct", "right", "definitely", "sure", "exactly", "confirmed", "yep", "yup"]
                    )
                    if _AFFIRM.match(_next) or _short_affirm:
                        _dest_confirmed = True
                        break
        if _dest_confirmed:
            filtered["destination_confirmed"] = True
        # Never set destination_confirmed to False here — once confirmed it stays confirmed
...
def build_intel(profile):
    ...
    # OBC is a post-booking topic — only relevant once they're close to pulling
    # the trigger, not during early planning even if a shortlist exists.
    booking_ready = profile.get("planning_stage") == "ready_to_book"
    if late_stage and booking_ready and (profile.get("cruise_experience") in ("experienced", "frequent_traveler") or profile.get("lines_sailed")):
        intel.append({
    ...
    # FCDs are bought ONBOARD a current sailing — only relevant for ready-to-book
    # travelers who may be going on a ship soon and want to prep.
    cruise_exp = (profile.get("cruise_experience") or "").lower()
    if late_stage and booking_ready and (cruise_exp in ("experienced", "frequent_traveler") or (profile.get("lines_sailed") and len(profile.get("lines_sailed") or []) > 1)):
        intel.append({
...
@app.route("/roi")
def roi_page():
    """Peregrine ROI Calculator for travel agencies."""
...
def session_new():
    """Start a fresh conversation without restoring any prior session data."""
    session.pop("session_id", None)
    session.pop("email_asked", None)
    session.pop("email_declined", None)
    session.pop("pending_email_matches", None)
    # Mark that the user explicitly chose a new cruise so that email-based
    # session restoration is skipped if they re-enter a known address.
    session["new_cruise_mode"] = True
...
@app.route("/api/panel-refresh")
def panel_refresh_route():
    """Consolidated endpoint for /api/profile + /api/alerts + /api/intel + /api/port.

    Added 2026-07-02 (Peregrine K15): the frontend was calling those four routes
    together after every chat turn, each independently re-fetching the same
    voyage_profiles row from Supabase — up to 4 redundant reads per turn. This
    endpoint calls get_profile() ONCE and derives whichever pieces are asked
    for via ?parts=profile,alerts,intel,port (defaults to all four).

    video is deliberately NOT included here — /api/video has its own extra
    Supabase query (the videos table) and non-trivial matching logic, so
    folding it in would only save the one get_profile() call while adding
    real refactor risk. Left as its own endpoint; still called separately
    by the frontend at whatever timing it already used.

    Each piece is wrapped in its own try/except so one piece failing returns
    an empty/default value for just that piece, matching the existing
    per-route error-handling behavior of the four routes above — a caller
    switching from the old routes to this one sees identical behavior on
    partial failure, not a single all-or-nothing error.
    """
    requested = (request.args.get("parts") or "profile,alerts,intel,port").split(",")
    requested = {p.strip() for p in requested if p.strip()}

    result = {}
    session_id = session.get("session_id")
    if not session_id:
        if "profile" in requested: result["profile"] = {}
        if "alerts"  in requested: result["alerts"]  = []
        if "intel"   in requested: result["intel"]   = []
        if "port"    in requested: result["port"]    = None
        return jsonify(result)

    # Single shared fetch — this is the whole point of this endpoint.
    try:
        profile = get_profile(session_id)
    except Exception:
        profile = {}
    ...
    if "profile" in requested:
        try:
            result["profile"] = _clear_stale_travel_year(profile or {})
        except Exception:
            result["profile"] = {}
    ...
    if "alerts" in requested:
        try:
            all_alerts = build_alerts(profile)
            result["alerts"] = [a for a in all_alerts if not a.get("advisor_only")]
        except Exception:
            result["alerts"] = []

    if "intel" in requested:
        try:
            result["intel"] = build_intel(profile)
        except Exception:
            result["intel"] = []

    if "port" in requested:
        try:
            ...
            if not port_name:
                ...
                result["port"] = rows[0] if rows else None
        except Exception:
            result["port"] = None
    ...
    try:
        ...
        if not video and destination:
            ...
            DEST_MAP = {
                ...
                "norway":    ("destinations/nyhavn-copenhagen.mp4",    "Norway",                     "your destination"),
                "fjord":     ("destinations/nyhavn-copenhagen.mp4",    "Norwegian Fjords",           "your destination"),
                ...
                # vancouver: departure port only — not a destination, excluded from DEST_MAP to avoid
                #            stale terminal video persisting when a real destination is later established.
            }
...
def chat():
    ...
    if EMAIL_RE.match(stripped_msg):
        ...
        # Don't restore a prior session if the user explicitly started a new cruise.
        looks_unstarted = (not existing_profile.get("destination_region")
                           and not existing_profile.get("first_name")
                           and not session.get("new_cruise_mode"))
        if looks_unstarted:
        ...
        if len(candidates) == 1:
            ...
            # A5+C6: no match or ambiguous reply — clear stuck state and move on fresh.
            session.pop("pending_email_matches", None)
            returning_user_note = (
                "\n\n[SYSTEM NOTE: The user responded to the saved-trip question "
                "but their reply did not clearly identify one of the options. "
                "Do not ask about saved trips again. Proceed as a new conversation "
    ...
    try:
        ...
        resp = claude.messages.create(
            ...
            max_tokens=800,
            system=build_system_blocks(profile, turn_count=turn_count),
    ...
    # Check for handoff intent. NOTE: this reuses the `profile` fetched earlier
    # in this same request rather than re-querying Supabase — extraction runs
    # in a background thread that hasn't written anything yet by this point
    # (it just started), so a fresh fetch here always returns identical data
    # to `profile` above. Re-fetching was a redundant Supabase round-trip on
